...
namespace MarioGame
{
    public class Camera
    {
        ...
        {
            Position = position - new Vector2(_viewport.Width / 2.0f, _viewport.Height / 2.0f); //+ Origin;
        }

        public void Move(Vector2 displacement, bool respectRotation = false)
        { //I don't think  we need to use this method because out camera is following Mario
            if (respectRotation)
            {
